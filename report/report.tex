...
\newcommand{\safeincludegraphics}[2][]{%
  \IfFileExists{#2}{\includegraphics[#1]{#2}}{\IfFileExists{../#2}{\includegraphics[#1]{../#2}}{\fbox{\parbox[c][3.2cm][c]{\linewidth}{\centering \small \texttt{[Figure pending generation: #2]}}}}}%
}
...
Standard supervised learning assumes that training and test distributions are identical and share the same label space. In real-world deployment, models encounter appearance shifts across domains and inputs from novel classes. This paper evaluates how representations behave under these distribution shifts across four settings: inductive biases, unsupervised domain adaptation, domain generalization, and open-set recognition. Across the four tasks, this paper investigates the following core research questions:
\begin{enumerate}
...
\begin{figure}[htbp]
    \centering
    \safeincludegraphics[width=0.72\linewidth]{figures/task1/cue_conflict_failures.png}
...
\begin{figure}[htbp]
    \centering
    \safeincludegraphics[width=0.72\linewidth]{figures/task1/patch_shuffle_examples.png}
...
\subsection{Methodology}
Unsupervised Domain Adaptation (UDA) addresses the scenario where a model is trained on labeled source domains and must adapt to an unlabeled target domain with differing visual statistics. For our Unsupervised Domain Adaptation experiments, we made use of the PACS dataset, which consists of seven classes across four domains (Photo, Art Painting, Cartoon, Sketch). We used Sketch as our unlabeled target domain. For each source domain, we used an 80/20 training/validation split with a seed of 6304.
...
In order to perform DAN-style alignment, we added a 512-dimensional Maximum Mean Discrepancy (MMD) penalty to the feature space: $L_{\text{DAN}} = L_{\text{cls}} + \lambda_{\text{MMD}} \text{MMD}^2(F(X_s), F(X_t))$, with $\lambda_{\text{MMD}}=1$ and a sum of three RBF kernels whose bandwidths were 0.5, 1, and 2 times the median pairwise squared feature distance in the current combined batch.
...
\begin{figure}[htbp]
    \centering
    \safeincludegraphics[width=0.72\linewidth]{figures/task2/confusion_failures.png}
...
\subsection{Methodology}
Domain Generalization (DG) evaluates whether a model trained on multiple source domains can generalize to an unseen target domain without receiving any target data during training. Under the Domain Generalization setup, the target domain (Sketch) was strictly unseen during all stages of training, validation, model selection, and hyperparameter tuning. We reused the shared PACS splits with an 80/20 train/validation split (seed 6304), fine-tuning a ResNet-18 model initialized with ImageNet weights and keeping BatchNorm running statistics frozen.
...
\begin{itemize}
    \item \textbf{GCSC (Good Closed-Set Classifier):} This method trains the modified ResNet-18 with aggressive RandAugment (2 operations, magnitude 9) under the vanilla recipe to test if stronger closed-set representation alone improves open-set detection.
    \item \textbf{PROSER (Learning Placeholders):} This method adds dummy classifier prototypes and dummy data placeholders generated via feature mixup between second-most-likely classes to explicitly carve out an unknown decision boundary. The backbone was fine-tuned for 50 epochs using SGD with learning rate $10^{-3}$, momentum 0.9, weight decay $5 \times 10^{-4}$, and cosine decay.
\end{itemize}
