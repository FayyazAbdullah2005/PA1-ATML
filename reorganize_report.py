...
# Read current report_skeleton.tex
with open('report_skeleton.tex', 'r', encoding='utf-8') as f:
    current_tex = f.read()

# Let's construct the reorganized LaTeX document with the exact user wording!
...
\end{figure}
\textit{[Discuss Figure~\ref{fig:task2_curves}: how source cross-entropy steadily declines while alignment objectives converge across epochs.]}
...
\end{figure}
\textit{[Discuss Table~\ref{tab:task2_main_results}, Table~\ref{tab:task2_per_class}, and Figure~\ref{fig:task2_confusions}: explain the source-to-target domain gap for Source-only, why DAN suffered negative transfer on Person, why DANN collapsed into predicting Dog (mode collapse), and why CDAN collapsed into predicting House due to positive error feedback.]}
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
\textit{[Discuss Table~\ref{tab:task2_design_study}: how increasing $\lambda$ reduces domain separability from 98.08\% to 85.71\% but causes target accuracy to degrade from 71.90\% to 64.29\%, showing that weak alignment ($\lambda=0.1$) is optimal and would be selected by source validation F1.]}
...
\end{figure}
\textit{[Discuss Table~\ref{tab:task3_main_results} and Figure~\ref{fig:task3_diagnostics}: whether source validation predicts Sketch accuracy, whether DAN-DG invariance helped or harmed, and how SAM sharp loss minimization correlates with target generalization.]}
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
\textit{[Discuss Table~\ref{tab:task3_per_class} and Table~\ref{tab:task3_design_study}: class-level differences between target-free DG vs target-aware UDA, and the effect of perturbation radius $\rho$ where $\rho=0.01$ achieved peak target accuracy of 75.74\%.]}
...
\end{figure}
\textit{[Discuss Table~\ref{tab:task4_posthoc_scores} and Figure~\ref{fig:task4_scoring_analysis}: how semantic proximity impacts novelty detection, why far unknowns are easier to reject than near unknowns, and the comparison between MSP, MLS, Energy, and Mahalanobis.]}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        Near & bus & truck & -12.05 & -5.85 & Plausible semantic overlap \\
        Near & wolf & bird & -11.36 & -5.85 & Plausible semantic overlap \\
        Near & fox & dog & -11.36 & -5.85 & Plausible semantic overlap \\
        \midrule
        Far & keyboard & airplane & -12.39 & -5.85 & Surprising feature alias \\
        Far & sunflower & bird & -11.80 & -5.85 & Surprising feature alias \\
        Far & mushroom & bird & -11.03 & -5.85 & Surprising feature alias \\
        \bottomrule
    \end{tabular}
\end{table}
\textit{[Discuss Table~\ref{tab:task4_trained_models}, Figure~\ref{fig:task4_failures}, and Table~\ref{tab:task4_failure_cases}: evaluate whether GCSC and PROSER improved rejection and closed-set accuracy, and analyze the specific failure cases (semantic overlap like bus-truck vs feature aliasing like keyboard-airplane).]}
...
\textit{[Synthesize representation stability and domain alignment across tasks: compare representation cosine stability in Task 1 with domain separability in Tasks 2 and 3. Address why domain invariance does not imply class-discriminative feature preservation, connecting DANN's mode collapse on PACS with Task 1's observation that feature shifts do not directly correlate with classifier decisions.]}

\subsection{Target Access vs. Robust Optimization}
\textit{[Contrast target-aware UDA (Task 2) with target-free DG (Task 3): explain why SAM without target data outperformed target-aligned DANN and CDAN, and discuss the value and danger of unlabeled target data under severe distribution shift.]}

\subsection{Closed-Set Discrimination vs. Open-Set Rejection}
\textit{[Connect closed-set performance to open-set detection (Task 4): discuss why high source or closed-set accuracy does not guarantee out-of-distribution rejection, and how placeholder learning and margin calibration constrain the decision space.]}
...
print("Successfully restructured report_skeleton.tex!")
