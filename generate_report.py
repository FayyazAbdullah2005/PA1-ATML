...
\subsection{Experimental Setup and Clean Baseline Evaluation}
\label{sec:task1_setup}

\paragraph{Experimental Setup and Hypothesis}
...
\end{enumerate}
The linear classifier head for each backbone was then trained using the AdamW optimizer (learning rate $10^{-3}$, weight decay $10^{-4}$, and early stopping patience of 5). Along with these three models, OpenCLIP Zero-Shot classification was also recorded using the prompt \texttt{"a photo of a \{class\}"}. The accuracy, F1-scores, and Mean Max confidence of these models are shown in Table~\ref{tab:task1_clean_baseline}.
...
\subsection{Chromatic Reliance vs. Shape and Texture Bias}
\label{sec:task1_color_shape_texture}

\paragraph{Experimental Setup and Hypotheses}
Since experiments conducted by \citet{geirhos2019imagenet} on texture bias in CNNs showed that grayscaling images did not result in a significant drop in performance for color interventions, we similarly expect little or no difference in performance for ResNet here. Since ViT and OpenCLIP learn global features and are trained on large datasets, we expect these models to also show only small drops in performance. We quantify performance changes using accuracy change ($\Delta \text{Acc}$) and prediction consistency. To test color bias, images were passed through a grayscale filter as the first test and a $180^\circ$ Hue rotation as a second test (evaluated across the 500 clean test images, seed 6304), with results reported in Table~\ref{tab:task1_color}.

\citet{geirhos2019imagenet} found a strong bias towards local textures in CNNs and \citet{raghu2021vision} found that ViTs produce more uniform representations across layers, so we expect CNNs to be more texture-biased and ViTs to be more shape-biased. Evaluated metrics are Shape Bias (\%) and Coverage (\%). A total of 220 cue-conflict images were created using AdaIN style transfer ($\alpha=0.8$) across 5 bidirectional class pairs, filtered by pre-evaluation visual rejection criteria ($\text{SSIM} \ge 0.35$, Sobel edge correlation $\ge 0.40$), as shown in Figure~\ref{fig:task1_stylization}. Model predictions were categorized into shape, texture, or third-class (other) decisions, with results summarized in Table~\ref{tab:task1_shape_bias}.
...
\subsection{UDA Protocol, Training Dynamics, and Baseline Domain Gap}
\label{sec:task2_setup}

\paragraph{PACS Dataset and Adaptation Protocol}
...
\paragraph{Architecture and Training Specifications}
...
\subsection{Domain Separability vs. Target Recognition: Invariance vs. Negative Transfer}
\label{sec:task2_separability_eval}

\paragraph{Method Formulation and Experimental Setup:}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccccccccc}
        \toprule
        & \multicolumn{3}{c}{\textbf{Source Val Accuracy (\%)}} & \textbf{Mean Source} & \textbf{Mean Source} & \textbf{Target (Sketch)} & \textbf{Target (Sketch)} & \textbf{Target Gain} & \textbf{Domain Sep.} \\
        \cmidrule(lr){2-4}
        \textbf{Method} & \textbf{Photo} & \textbf{Art} & \textbf{Cartoon} & \textbf{Accuracy (\%)} & \textbf{Macro-F1} & \textbf{Accuracy (\%)} & \textbf{Macro-F1} & \textbf{$\Delta$ Acc (\%)} & \textbf{Score (\%)} \\
        \midrule
        Source-only (ERM) & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
        DAN & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
        DANN & -- & -- & -- & -- & -- & -- & -- & -- & -- \\
...
\subsection{Marginal vs. Class-Conditional Alignment: Semantic Preservation}
\label{sec:task2_conditional_eval}
...
\subsection{Controlled Design Study: Alignment Strength Sensitivity}
\label{sec:task2_study}
...
\subsection{Domain Generalization Protocol and Optimization Strategies}
\label{sec:task3_setup}

\paragraph{Domain Generalization Protocol}
...
\subsection{Source Invariance vs. Unseen Generalization}
\label{sec:task3_evaluation}
...
\subsection{Parameter Stability and the Sharpness Proxy}
\label{sec:task3_sharpness}

\subsection{Target-Free DG vs. Target-Aware UDA \& Controlled Study}
\label{sec:task3_study}
...
\subsection{Experimental Protocol and Semantic Unknown Splits}
\label{sec:task4_setup}
...
\paragraph{Architecture and Training Protocol}
...
\subsection{Post-Hoc Novelty Scoring on Vanilla Classifier}
\label{sec:task4_posthoc}
...
\subsection{Representation \& Placeholder Learning: GCSC and PROSER}
\label{sec:task4_placeholders}

\paragraph{GCSC Protocol}
Write GCSC protocol here.

\paragraph{PROSER Protocol}
